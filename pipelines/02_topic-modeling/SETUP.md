...
| `result_2_isolated_papers.xlsx` | **Threshold-Based Isolation:** Isolates documents with a semantic isolation score of $\ge 0.99$, calculated via the equation: \$1 - \text{max\_prob}$. | Pins down highly unique manuscripts that exhibit virtually zero alignment or statistical affinity with the core corpus clusters. |
